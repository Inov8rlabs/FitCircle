...
| **3. Migration 083** (formerly 077/082; NOT applied today) | Free-tier *quotas* (5 AI parses, 5 Fitzy msgs, 14-day history, 2 circles, ads gating). Until applied, behavior is byte-identical to today. | `supabase/pending/083_enable_pro_gates.sql` |

Launch order is always **1 → (verify) → 3**. Turning on the flag before 083 means people can buy Pro while free users lose nothing — a safe soft-launch. 083 is the moment free limits begin.
...
- [ ] (Later, optional) Offer on `pro-annual`: `pro-annual-free-trial` — 7-day free phase, eligibility "never had this subscription". Not at launch.
...
- [ ] Create project "FitCircle" with three apps: iOS (`com.inov8rlabs.apps.fitcircle` — the Inov8r Labs Inc bundle id since 2026-09-12; NOT the old `com.inov8rlabs.FitCircle`), Android (`com.fitcircle.app`), Stripe
...
- [ ] Backend: `stripe listen --forward-to localhost:3000/api/webhooks/stripe`, complete test checkouts (monthly, annual, annual-with-coupon, lifetime; set `STRIPE_TRIAL_DAYS` only when running a trial promo) → confirm profile flips premium and `payments` rows appear once
...
- [ ] iOS sandbox: purchase both plans, renewal (accelerated), cancel, restore on reinstall, lifetime hides sub UI, ads disappear instantly on purchase
...
1. [ ] Flag → staged rollout (10% → 50% → 100%) while watching: purchases, renewals, webhook error logs, reconcile-cron corrected count (should stay ~0)
2. [ ] When conversion loop is proven end-to-end: apply **migration 083** (free-tier quotas begin; ads gate on entitlement; watch the guardrail metrics — K-factor ≥ 0.4, D1/D7/D30 ≥ 40/20/10, week-1 circle-join ≥ 35%; if they drop, loosen free limits before touching the paywall)
...
STRIPE_TRIAL_DAYS                       (optional, free-trial promo only; unset = no trial)
