-- ============================================================================
-- 078: STREAK SHIELD CONSOLIDATION
-- ============================================================================
...
-- 1. Ensure every user has the three shield rows
INSERT INTO streak_shields (user_id, shield_type, available_count)
SELECT p.id, t.shield_type, 0
FROM profiles p
...
COMMENT ON COLUMN engagement_streaks.streak_freezes_used_this_week IS
  'DEPRECATED (weekly freeze model removed in 078). Not read by current code.';
COMMENT ON COLUMN engagement_streaks.auto_freeze_reset_date IS
  'DEPRECATED (weekly freeze model removed in 078). Not read by current code.';
COMMENT ON COLUMN engagement_streaks.shields_available IS
  'DEPRECATED legacy field (superseded by streak_shields in 029, formally dead since 078).';
