-- Migration 008: FitCircle MVP Implementation (Fixed)
...
CREATE TABLE IF NOT EXISTS circle_invites (
    ...
    -- NOTE: Removed subquery CHECK constraint - validation will be done in application layer
);
...
CREATE POLICY "Circle creators and members can create invites"
    ...
    WITH CHECK (
        ...
        OR auth.uid() = (SELECT creator_id FROM challenges WHERE id = circle_id)
    );
