...
/**
 * GET /api/mobile/circles/[id]/streak
 * PRD v4 §6.13 — the circle's COLLECTIVE streak (consecutive days where most active
 * members logged). Active-member gated. Thin route; logic lives in the service.
 */
export async function GET(
  ...
  try {
    ...
    const streak = await CircleStreakService.getStreak(id, user.id);
    ...
    return mapError(error, 'circle-streak');
  }
}
